 ...
 */
function getNfseCertPasswordKey($db)
{
    ...
    if ($resql && $db->num_rows($resql) > 0) {
        ...
        // Usa INSERT IGNORE para ser idempotente (evita erro se chamada concorrente já inseriu)
        $sqlInsert = "INSERT IGNORE INTO ".MAIN_DB_PREFIX."nfe_config (name, value) 
                      VALUES ('encryption_master_key', '".$db->escape($masterKey)."')";
        $resInsert = $db->query($sqlInsert);
        
        // Se o INSERT foi ignorado (race condition), lê a chave já existente
        if ($resInsert && $db->affected_rows($resInsert) === 0) {
            $resRecheck = $db->query("SELECT value FROM ".MAIN_DB_PREFIX."nfe_config WHERE name = 'encryption_master_key'");
            if ($resRecheck && $db->num_rows($resRecheck) > 0) {
                $objRecheck = $db->fetch_object($resRecheck);
                $masterKey = $objRecheck->value;
            }
        }
    }
 ...
 */
function nfseEncryptPassword($password, $db)
{
    if ($password === '') {
        return '';
    }
    
    $key = getNfseCertPasswordKey($db);
 ...
 */
function nfseDecryptPassword($encryptedPassword, $db)
{
