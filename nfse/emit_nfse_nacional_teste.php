...
// Polyfills para helpers utilizados no script
...
try {
    ...
    $std->infDPS->tpAmb = 2; // 1 - Produção, 2 - Homologação
    $std->infDPS->dhEmi = now()->format('Y-m-d\TH:i:sP'); // "Data e hora da emissão da DPS. AAAA-MM-DDThh:mm:ssTZD"
    $std->infDPS->verAplic = '1.01';
    $std->infDPS->serie = 1;
    $std->infDPS->nDPS = 4;
    $std->infDPS->dCompet = now()->format('Y-m-d');
    echo $std->infDPS->dCompet; //"Data de competência da prestação do serviço. Ano, Mês e Dia (AAAA-MM-DD)"
    $std->infDPS->tpEmit = 1; //Emitente da DPS: 1 - Prestador; 2 - Tomador; 3 - Intermediário;
    $std->infDPS->cLocEmi = '3201209'; //Código IBGE 7 dígitos da cidade emissora da NFS-e.
    ...
    $std->infDPS->prest->CNPJ = '48681521000188';
...
//    $std->infDPS->prest->email = '';
    ...
    $std->infDPS->prest->regTrib->opSimpNac = 1; // Situação perante Simples Nacional: 1 - Não Optante; 2 - Optante - Microempreendedor Individual (MEI); 3 - Optante - Microempresa ou Empresa de Pequeno Porte (ME/EPP);
    //$std->infDPS->prest->regTrib->regApTribSN = 1; // Regime de Apuração Tributária pelo Simples Nacional. 1 – Regime de apuração dos tributos pelo SN; 2 – Regime de apuração dos tributos federais pelo SN e o ISSQN pela NFS-e conforme respectiva legislação municipal; 3 – Regime de apuração dos tributos federais e municipal pela NFS-e conforme respectivas legilações federal e municipal de cada tributo;"
    ...
    $std->infDPS->toma->CNPJ = '37242589000144';
    ...
    $std->infDPS->toma->xNome = 'Hadder Soft';
...
//    $std->infDPS->toma->email = '';
    ...
    $std->infDPS->toma->end->endNac->cMun = '3201209'; //Código IBGE 7 dígitos
    $std->infDPS->toma->end->endNac->CEP = '29310351';
    ...
    $std->infDPS->serv->cServ->xDescServ = 'Suporte';
     $std->infDPS->serv->cServ->cNBS = '123019200';// Código NBS correspondente ao serviço prestado
...
//    $std->infDPS->serv->infoCompl->xInfComp = 'Informações complementares';//Campo livre para preenchimento pelo contribuinte.
    ...
    $std->infDPS->valores->trib->totTrib->pTotTribSN = 0;

    $dps = new \Hadder\NfseNacional\Dps($std);
    
    $response = $tools->enviaDps($dps->render());
    file_put_contents('dps.xml', $dps->render());
    //    $response = [
    //        "tipoAmbiente" => 1,
